...
import { subscribeToKpiDetails, addKpiDetail } from '../services/firebaseService';
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {!isManager && (
            <div className="mb-6 flex gap-3 items-end p-4 bg-blue-50/50 rounded-xl border border-blue-100">
              ...
              <button 
                onClick={handleAdd}
                disabled={!newCustomer || newAmount === ''}
          ...
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  <tr><td colSpan={3} className="px-4 py-6 text-center text-slate-400">Đang tải...</td></tr>
                ) : details.length === 0 ? (
                  <tr><td colSpan={3} className="px-4 py-6 text-center text-slate-400">Chưa có phát sinh nào</td></tr>
                ) : (
                  details.map(d => (
