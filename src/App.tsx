  ...
  const renderDashboard = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        <h3 className="text-sm font-bold text-[#1a2b4b] uppercase tracking-wide border-b border-slate-200 pb-2 mt-8 mb-2">Số liệu Tổng chi nhánh</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {aggregateKpis.length === 0 && (
            <div className="col-span-full py-4 text-center text-slate-400 text-xs shadow-sm bg-white rounded-xl border border-slate-100 p-5 mt-1 mb-2">Chưa có chỉ tiêu chi nhánh.</div>
          )}
        </div>

        <Card title={currentUser.role === 'Manager' ? "Hiệu suất Tổng hợp toàn chi nhánh" : "Tiến độ thực hiện của tôi"}>
          <div className="h-80">
          ...
          <Card title={`Số liệu thực hiện Tháng ${selectedMonth}/${selectedYear}`}>
                  ...
                  <div key={kpi.id} className="p-5 rounded-2xl border border-slate-100 bg-white shadow-sm flex flex-col group hover:border-blue-200 transition-all">
                    ...
                    <div className="flex items-center gap-3 mb-4">
                      <div className="flex-1">
                        <p className="text-[9px] text-slate-400 font-bold uppercase mb-1">Thực hiện (Nhập số)</p>
                        <input 
                          type="number" 
                          value={kpi.actual}
                          onChange={(e) => handleKpiUpdate(kpi.id, Number(e.target.value), 'actual')}
                          className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl text-lg font-extrabold text-[#1a2b4b] outline-none focus:ring-2 focus:ring-blue-500 transition-all"
                        />
                      </div>
                      <div className="text-right pt-4">
                        ...
                        <button 
                          ...
                          className="mt-2 text-[10px] bg-slate-100 text-slate-600 px-2 py-1 rounded hover:bg-slate-200 transition-colors font-medium border border-slate-200"
                        >
                          Chi tiết KH
                        </button>
                      </div>
            ...
            <div className="mt-8 flex justify-between items-center bg-blue-50 p-4 rounded-xl border border-blue-100">
               <div className="flex items-center gap-3 text-blue-700">
                  <TrendingUp size={20} />
                  <span className="text-xs font-bold">Dữ liệu được đồng bộ thời gian thực với quản lý chi nhánh</span>
               </div>
               <button 
                                 ...
                                 {kpi ? (
                                   <div className="space-y-1">
                                     <input 
                                       type="number" 
                                       value={kpi.target}
                                       onChange={(e) => handleKpiUpdate(kpi.id, Number(e.target.value), 'target')}
  ...
  const renderStats = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        <Card title="Biểu đồ tiến độ KPIs toàn chi nhánh (Định mức tỷ VNĐ / Thẻ)">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input 
                    ...
                    onChange={e => setNewDossier({...newDossier, amount: Number(e.target.value)})}
                  />
                </div>
              </div>
              <div className="flex gap-3 mt-8">
                ...
                {filteredDossiers.length === 0 ? (
                  ...
                  return (
                    <tr key={hs.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="px-5 py-3 text-right font-mono font-medium text-slate-900">{hs.amount.toLocaleString()}</td>
